...
while True:
    ...
    if results:
        ...
        try:
            ...
            gat_num, dem_gat, prev_status = gat_dau(prev_status, mode, dem_gat, gat_num, mode)
            # md.write(str(mode/10)+"\n")
            # avg.write(str(round(ty_le_tb,3))+"\n")
            if canh_bao:
                cv2.putText(img, "CANH BAO!!!", (int(iw/2)-200, int(ih/2)), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 1)
