...
def trang_thai_mat(ty_le_mat, ty_le_mat_phai, ty_le_mat_trai, mode, trang_thai_trc):
    if mode == 0:
        if ty_le_mat <= 0.25:
            trang_thai = 'Nham'
            # dem += 1
        ...
        if ty_le_mat <= 0.25:
            ...
            # canh_bao = False
    trang_thai_trc = trang_thai
    return trang_thai, trang_thai_trc
...
def canh_bao_buon_ngu(trang_thai, dem, canh_bao):
    if trang_thai == "Nham":
        dem+=1
        if dem >= 25:
            canh_bao = True
